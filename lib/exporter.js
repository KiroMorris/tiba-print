...
// Keep memory under control for huge print sheets:
//  • disable libvips' operation cache (it holds big intermediates in RAM)
//  • limit concurrency so we don't run many large ops at once
//  • lift the pixel-count safety limit (our sheets are intentionally huge)
sharp.cache(false);
sharp.concurrency(1);
sharp.simd(true);
...
export async function exportJob(result, { dpi = 300, format = 'png', jobName = 'job' } = {}) {
  ...
  for (const bin of result.bins) {
    ...
    // Guard: warn (don't crash) if a single sheet is enormous. ~715M px is the
    // practical ceiling for an in-one-pass composite; beyond that, TIFF (tiled,
    // streamed) is the only safe format.
    const totalPx = rollWpx * usedLenPx;
    if (totalPx > 715_000_000 && format !== 'tiff') {
      throw new Error(
        `This print file is very large (${(totalPx / 1e6).toFixed(0)} megapixels at ${dpi} DPI). ` +
        `Export as TIFF (memory-safe for big sheets), lower the DPI, or split into shorter files.`
      );
    }
    ...
    if (format === 'png') {
      // compressionLevel 9 is very memory-hungry on huge images; level 6 is the
      // sweet spot (near-identical size, far less RAM). toFile() streams to disk.
      await canvas.png({ compressionLevel: 6 }).withMetadata({ density: dpi }).toFile(absOut);
    } else if (format === 'tiff') {
