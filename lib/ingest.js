...
// Returns { png, width, height, dpi }. For raster files dpi is the real embedded
// density; for vector (SVG/PDF) dpi is null because they scale infinitely (no
// resolution loss), so they're never flagged as low-res.
async function toRasterPng(buffer, type) {
  if (type === 'png' || type === 'jpg') {
    const img = sharp(buffer, { limitInputPixels: false });
    const meta = await img.metadata();
    const png = await img.png().toBuffer();
    return { png, width: meta.width, height: meta.height, dpi: meta.density || null };
  }
  ...
  if (type === 'svg') {
    ...
    return { png, width: rendered.width, height: rendered.height, dpi: null }; // vector
  }

  if (type === 'pdf') {
    const png = await rasterizePdfFirstPage(buffer);
    const meta = await sharp(png).metadata();
    return { png, width: meta.width, height: meta.height, dpi: null }; // vector
  }
...
export async function ingestFile(buffer, filename) {
  ...
  const { png, width, height, dpi } = await toRasterPng(buffer, type);
  const bounds = await contentBounds(png, width, height);

  // Real DPI handling:
  //  • Vector (SVG/PDF) → dpi is null → scales infinitely, treat as 300, no flag.
  //  • Raster with a real embedded DPI ≥ 300 → use it (correct physical size).
  //  • Raster with DPI < 300, or no/default density (72) → the file is low-res:
  //    flag "needs enhancement", and use 300 for the SIZE math so the design
  //    doesn't balloon huge. We still record the TRUE source DPI for reference.
  const isVector = dpi == null;
  const realDpi = isVector ? 300 : Math.round(dpi);
  // A bare 72 almost always means "no DPI set", not a genuine 72-DPI design.
  const hasMeaningfulDpi = !isVector && realDpi !== 72;
  const needs_enhance = !isVector && (!hasMeaningfulDpi || realDpi < 300) ? 1 : 0;

  // DPI used to convert pixels → cm. Never go below 300 so low-res files keep a
  // sane physical size (they just print soft, hence the flag).
  const sizingDpi = isVector ? ASSUME_DPI : Math.max(realDpi, ASSUME_DPI);

  const phys_w_cm = +((bounds.trim_w / sizingDpi) * 2.54).toFixed(2);
  const phys_h_cm = +((bounds.trim_h / sizingDpi) * 2.54).toFixed(2);
  ...
  return {
    ...
    phys_w_cm,
    phys_h_cm,
    src_dpi: isVector ? 0 : realDpi, // 0 = vector / not applicable
    needs_enhance,
  };
}
